...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player#no need of plus one so the column index gets positioned correctly according to the code-edited by svetlana
    return game
...
def switch_player(player):
    if player == 1:#use comparison operator == instead of = which means if player is equal to 1-edited by svetlana
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #colon has been added for the code below to be executed in the loop-edited by svetlana
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column)#column has been added-edited by svetlana
